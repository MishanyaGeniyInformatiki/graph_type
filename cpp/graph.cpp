...
{	
	...
	while (!q.empty()){
		...
		for (auto&& elem : graph.adjLists_[vert]){ // рассматриваю все вершины смежные с vert
		...
		long min_dist = INF;
		for (int i = 0; i < numvert; i++){ // выбираю на роль следующей вершины вершину с минимальным dist 
	...
	for (int i = 0; i < numvert; i++) { // печатаю расстояния
		cout << "Distance from " <<  start << " vertex to " << i << " vertex = " <<  dist[i] << endl;
	}
}